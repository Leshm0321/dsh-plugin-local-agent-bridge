...
export const zh = {
  ...
  'usage.tokens': '上下文 {used}',
  'usage.ofWindow': '上下文 {used} / {max}（{percent}%）',
  'usage.title': '上下文用量',

  // Permission modes, worded after the Claude desktop app so the vocabulary
  // matches what operators already know.
  'opening.lead': '这个会话还没开始。它会这样运行：',
  'opening.product': '产品',
  'opening.where': '工作目录',
  'opening.mode': '权限模式',
  'opening.slash': '列出这个产品自己的命令与 Skill',
  'opening.at': '引用工作目录里的文件',
  'opening.paste': '直接粘贴图片，Agent 就能看到',
  'mode.label': '模式',
  'mode.auto': '自动',
  ...
  'model.defaultResolved': '沿用你在 CLI 里配置的模型 — 目前是 {name}',
  'model.resolved': '{name} — 产品自己的默认，不是在这里选的',
  ...
  // Lightweight file management in the side panel.
  'files.edit': '编辑这个文件',
  'files.save': '保存',
  'files.cancel': '取消',
  'files.confirm': '确定',
  'files.newFile': '在根目录新建文件',
  'files.newDirectory': '在根目录新建目录',
  'files.newIn': '在 {name} 里新建文件',
  'files.rename': '重命名 {name}',
  'files.delete': '删除 {name}',
  'files.nameIt': '输入名称，回车确定',
  'files.renameTo': '输入新名称',
  'files.stale': '这个文件在你打开之后被改动过（很可能是 Agent 改的）。重新打开它，再决定保留哪一版。',
...
export const en = {
  ...
  'usage.tokens': 'context {used}',
  'usage.ofWindow': 'context {used} / {max} ({percent}%)',
  'usage.title': 'Context used',

  'opening.lead': 'Nothing has been asked yet. This session will run like this:',
  'opening.product': 'Product',
  'opening.where': 'Working directory',
  'opening.mode': 'Permission mode',
  'opening.slash': 'lists this product\'s own commands and skills',
  'opening.at': 'references a file in the working directory',
  'opening.paste': 'Paste an image and the agent sees it',
  'mode.label': 'Mode',
  'mode.auto': 'Auto',
  ...
  'model.defaultResolved': 'Keep whatever you configured in the CLI — currently {name}',
  'model.resolved': '{name} — the product\'s own default, not a choice made here',
  ...
  'turn.waited': 'plus {value} waiting on you',
  ...
  'files.newFile': 'New file in the root',
  'files.newDirectory': 'New directory in the root',
  'files.newIn': 'New file in {name}',
  'files.rename': 'Rename {name}',
  'files.delete': 'Delete {name}',
  'files.nameIt': 'Type a name, then Enter',
  'files.renameTo': 'Type the new name',
