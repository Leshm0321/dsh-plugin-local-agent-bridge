...
| --- | --- | --- |
| DeepSeek Harness | `0.1.6-alpha.2` | External Host + Client plugin APIs targeted at commit `ddefc45fbc7f8e46dd73185e68295696d1297887`. Build, typecheck, lint, and the automated suite pass against it, and a booted Web Profile confirmed the panel loads, groups its sessions, and still resolves the directory browser. This is the `alpha` tag — a weaker claim than the `rc` line below it, taken deliberately: `latest` and `next` both sit at `0.1.5-rc.2`, a minor behind. The one break this release brought to the surfaces used here was `TypertCodec`, whose `schema` field became a `create()` factory. |
