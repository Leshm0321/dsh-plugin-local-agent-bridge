...
| 面 | 版本 | 策略 |
| --- | --- | --- |
| DeepSeek Harness | `0.1.5-rc.2` | 对外的 Host 与 Client 插件 API 面向提交 `fb2c4b9e698e30edb738bca4cf0618587db7d203`。构建、类型检查、lint 与自动化测试套件在其上全部通过，并且真实启动过一个 Web Profile 确认面板加载、目录浏览仍能解析。锁的是 `next` 标签而非 `latest`：`latest` 指向的 `0.1.5-rc.1` 装不出一致的依赖图 —— 传递范围会拉到 `rc.2`，留下四个未满足的 peer。 |
