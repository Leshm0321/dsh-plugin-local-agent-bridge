...
| 面 | 版本 | 策略 |
| --- | --- | --- |
| DeepSeek Harness | `0.1.6-alpha.1` | 对外的 Host 与 Client 插件 API 面向提交 `0a15e36e7f82b6ed45af6fa9759f29b40dcd965d`。构建、类型检查、lint 与自动化测试套件在其上全部通过，并且真实启动过一个 Web Profile 确认面板加载、会话分组、目录浏览仍能解析。这是 `alpha` 标签 —— 比它取代的 `rc` 线是更弱的保证，属于有意选择。`latest` 仍指向 `0.1.5-rc.1`，而那一套根本装不出一致的依赖图：传递范围无论如何都会拉到 `rc.2`。 |
