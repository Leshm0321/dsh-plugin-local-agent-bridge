...
| **会话** | 按各自运行的工作目录分组，可逐组折叠。以首次提问命名，可重命名、可置顶，列表变长后可筛选；面板关闭期间某轮结束会在侧边栏入口打上角标 |
...
本项目使用 MIT 许可证。Codex App Server schema 由 Codex `0.155.1` 生成；Codex 使用 Apache-2.0 许可证分发。Claude Agent SDK 声明 `SEE LICENSE IN README.md`，Anthropic 则说明 Agent SDK 受其商业条款约束。本项目不分发任何厂商二进制文件，也不授予对厂商服务的任何权利。
